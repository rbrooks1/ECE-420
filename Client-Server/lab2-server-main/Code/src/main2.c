...
char **strings;
pthread_mutex_t *strings_mutexes;
 ...
 * or write to a global array of strings which is guarded by one mutex per string.
 *
 * When running main2, three parameters are expected for string array length, server IP address, and server port,
...
int main(int argc, char *argv[]) {
    ...
    strings_mutexes = malloc(num_strings * sizeof(pthread_mutex_t));
    for (int i = 0; i < num_strings; ++i) {
        strings[i] = (char *) malloc(COM_BUFF_SIZE * sizeof(char));
        sprintf(strings[i], "String %d: the initial value", i);

        pthread_mutex_init(&(strings_mutexes[i]), NULL);
    }
 ...
 *
 * One mutex per string is used to synchronize access to the string array.
 */
void *handle_request(void *args) {
    ...
    if (client_request.is_read) {
        pthread_mutex_lock(&(strings_mutexes[client_request.pos]));
        getContent(server_msg, client_request.pos, strings);
        pthread_mutex_unlock(&(strings_mutexes[client_request.pos]));
