...
char **strings;
mylib_rwlock_t rwlocks[COM_NUM_REQUEST];
 ...
 * or write to a global array of strings which is guarded by one read-write lock per string.
 *
 * When running main4, three parameters are expected for string array length, server IP address, and server port,
 ...
 *
 * One read-write lock per string is used to synchronize access to the string array.
 */
void *handle_request(void *args) {
    ...
    if (client_request.is_read) {
        mylib_rwlock_rlock(&(rwlocks[client_request.pos]));
        getContent(server_msg, client_request.pos, strings);
        mylib_rwlock_unlock(&(rwlocks[client_request.pos]));
    } else { // Assume write operation
        mylib_rwlock_wlock(&(rwlocks[client_request.pos]));
